...
!          read *, nx
      ...
      call MPI_BCAST(nx,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr) ! Broadcast nx to each process
      ny   = nx
!
...
!        if (myid .eq. 0) print *, 2*it, ' Difference is ', diffnorm
      enddo
      if (myid .eq. 0 .and. it .gt. 1000) print *, 'Failed to converge'
      t2 = MPI_WTIME()
      if (myid .eq. 0) then
...
!     
      OPEN(unit=15, FILE='output.dat', STATUS='REPLACE', ACCESS='APPEND', &
